...
class Gui:
    ...
    def __init__(self, data_consumption, data_meter):
        ...
        self.visualise_button = ctk.CTkButton(
            self.root, text="Visualisieren", command=self.choose_visualise
        )
        self.visualise_button.pack(pady=(50, 10))

        self.add_files_button = ctk.CTkButton(
            self.root, text="Add Files", command=self.show_add_files_options
        )
        self.add_files_button.pack(pady=(10, 10))

        self.export_button = ctk.CTkButton(
            self.root, text="Exportieren", command=self.choose_export
        )
        ...
        self.obis_dropdown = ctk.CTkComboBox(
            self.root,
            state="readonly",
            values=["ID735", "ID742"],
            variable=self.obis_var,
        )
    ...
    def choose_export(self):
        ...
        if not self.filedialog:
            messagebox.showwarning(
                "No Directory", "Please select a directory to export."
            )
            return
    ...
    def show_add_files_options(self):
        ...
        self.esl_button = ctk.CTkButton(
            self.root, text="New ESL-File", command=self.add_new_esl_file
        )
        self.esl_button.pack(pady=(100, 10))
        self.sdat_button = ctk.CTkButton(
            self.root, text="New SDAT-File", command=self.add_new_sdat_file
        )
    ...
    def export(
        ...
        obiscode: str,
        export_type: str,
        data_consumption: list[ConsumptionData],
        data_meter: list[MeterData],
    ):
        """Docstring"""
        exporter = Exporter()
        if export_type == "csv":
            exporter.export_to_csv(path, obiscode, data_consumption, data_meter)
            print("csv exported")
        elif export_type == "json":
            exporter.export_to_json(path, obiscode, data_consumption, data_meter)
            print("json exported")
        else:
